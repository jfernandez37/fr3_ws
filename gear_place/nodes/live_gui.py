...
class FR3_GUI(tk.Tk):
    def __init__(self):
        ...
        # exit flag
        self.exit_flag = tk.StringVar()
        self.exit_flag.set('0')
        ...
        self.scan_already = False
        self.distances_from_home,self.updated_radius_vals = [],[]
        self.supervisor = GearPlace()
        self.supervisor.wait(5)
        self.supervisor.call_open_gripper_service()
    
    def pack_and_append(self, widget, pady=5, side = tk.TOP):
        widget.pack(pady=pady,side=side)
        self.current_widgets.append(widget)

    def exit_function(self):  # exits at any point in the program
        if self.conveyor_enabled:
            self.supervisor.enable_conveyor_service(False)
        self.destroy()
    ...
    def save_command(self):
        ...
        if self.parameters["command_type"].get()=="open_gripper":
            self.supervisor.call_open_gripper_service()
        elif self.parameters["command_type"].get()=="cartesian_movement":
            if self.parameters["type"]=="standard":
                self.supervisor.call_move_cartesian_service(float(self.parameters['x'].get()),float(self.parameters['y'].get()),float(self.parameters['z'].get()),float(self.parameters['v_max'].get()),float(self.parameters['acc'].get()))
            elif self.parameters["type"]=="angle":
                self.supervisor.call_move_cartesian_angle_service(float(self.parameters['x'].get()),float(self.parameters['y'].get()),float(self.parameters['z'].get()),float(self.parameters['v_max'].get()),float(self.parameters['acc'].get()),float(self.parameters['angle'].get()))
            else:
            ...
            self.supervisor.call_pick_up_gear_service((True if self.parameters['depth_or_color'].get()==CAMERA_TYPES[0] else False),float(self.parameters['object_width'].get()),self.parameters['starting_position'].get())
        elif self.parameters["command_type"].get()=="pick_up_multiple_gears":
            if not self.scan_already:
                self.distances_from_home,self.updated_radius_vals = self.supervisor.select_scan(type_scan="grid")
            comma_needed=False
            color_list = []
            if self.parameters["yellow"].get()=='1':
                color_list.append("yellow")
                comma_needed=True
            if self.parameters["orange"].get()=='1':
                if comma_needed:
                    color_list+=","
                color_list.append("orange")
                comma_needed=True
            if self.parameters["green"].get()=='1':
                if comma_needed:
                    color_list+=","
                color_list.append("green")
            self.supervisor.pick_up_multiple_gears(self.distances_from_home,self.updated_radius_vals,float(self.parameters['object_width'].get()),self.parameters['starting_position'].get(),color_list,(True if self.parameters['depth_or_color'].get()==CAMERA_TYPES[0] else False),self.parameters['put_down_type'].get(),float(self.parameters['force'].get()),self.parameters['put_down_pose'].get())
        elif self.parameters["command_type"].get()=="put_down_gear":
            self.supervisor.put_gear_down_choose_type(self.parameters['put_down_type'].get(),float(self.parameters['z'].get()),float(self.parameters['force'].get()))
        elif self.parameters["command_type"].get()=="moving_gears":
            if self.parameters["movement_type"]=="pick_up:":
                self.supervisor.call_move_up_moving_gear_service(float(self.parameters['object_width'].get()))
            else:
                self.supervisor.call_move_above_gear()
        elif self.parameters["command_type"].get()=="move_to_named_pose":
            self.supervisor.call_move_to_named_pose_service(self.parameters['name_pose'].get())
        elif self.parameters["command_type"].get()=="enable_conveyor":
            self.conveyor_enabled = True
            self.supervisor.enable_conveyor_service(True)
        elif self.parameters["command_type"].get()=="disable_conveyor":
            self.conveyor_enabled = False
            self.supervisor.enable_conveyor_service(False)
        ...
        elif self.parameters["command_type"].get()=="move_to_joint_position":
            self.supervisor.call_move_to_joint_position([float(self.parameters[f'joint_position_i'].get()) for i in range(7)])
