...
{
public:
  ForceMotionGenerator(double force, double max_travel, franka::Model &model, franka::RobotState &state);
  franka::Torques operator()(const franka::RobotState &robot_state, franka::Duration period);
  bool get_result() { return entered_; };
...
ForceMotionGenerator::ForceMotionGenerator(double force, double max_travel, franka::Model &model, franka::RobotState &state)
    : force_(force), max_travel_(max_travel), model_(model), state_(state), initial_tau_ext_(7), tau_error_integral_(7)
{
                                                 ...
                                                 franka::Duration period)
{
  ...
  // else if (abs(current_position[2] - initial_position_[2]) > max_travel_)
